# Server Mods

These jars are required and installed on the dedicated/local server, but they are not tracked by Git. Mod jars are managed manually for now.

JourneyMap, Waila/Hwyla, and Waila Harvestability are intentionally present in the server mod list for this pack and should not be assumed client-only here.

```text
!mixinbooter-10.7.jar
!Red-Core-MC-1.8-1.12-0.7.1.jar
_MixinBootstrap-1.1.0.jar
`FermiumBooter-1.4.1.jar
+Fugue-0.23.6.jar
Advanced Solar Panels-4.3.0.jar
AdvSolarPatch-1.2.1.jar
ae2stuff-0.7.0.4-mc1.12.2.jar
ae2-uel-v0.56.7.jar
Alfheim-1.6.jar
bdlib-1.14.4.1-mc1.12.2.jar
BrandonsCore-1.12.2-2.4.20.162-universal.jar
buildcraft-all-8.0.0.jar
CarbonConfig-1.12.2-2.0.2.jar
CodeChickenLib-1.12.2-3.2.3.358-universal.jar
CoFHCore-1.12.2-4.6.6.1-universal.jar
CoFHWorld-1.12.2-1.4.0.1-universal.jar
CraftTweaker2-1.12-4.1.20.698.jar
Draconic-Evolution-1.12.2-2.3.28.354-universal.jar
EnergyControl-1.12.2-0.3.2.jar
fermiumasm-5.31.jar
FermiumMixins-1.0.13.jar
Fixeroo-2.3.7.jar
forestry_1.12.2-5.8.2.427.jar
Forgelin-Continuous-2.3.20.0.jar
FTBBackups-1.1.0.1.jar
generators-0.9.20.12-mc1.12.2.jar
GraviSuite-Patcher-1.0.0.jar
Gravitation Suite-3.1.1.jar
HadEnoughItems_1.12.2-4.31.2.jar
Hwyla-1.8.26-B41_1.12.2.jar
IC2-Patcher-2.0.13.jar
ic2-tweaker-0.2.1+build.4.jar
industrialcraft-2-2.8.222-ex112.jar
industrialforegoing-1.12.2-1.12.13-237.jar
InventoryTweaks-1.64+dev.151.jar
ironchest-1.12.2-7.0.72.847.jar
journeymap-1.12.2-5.7.1p3.jar
LibrarianLib-Continuous-1.12.2-4.22-2.0-3.jar
modtweaker-4.0.20.11.jar
MoreTweaker-1.0.19.jar
mousetweaks-1.12.2-3.1.5.jar
MTLib-3.0.7.jar
NoiseThreader-1.1.3.jar
PaperFixes-2.0.1.jar
Patchouli-1.0-23.6.jar
railcraft-12.1.0-beta-8.jar
RailwayDepotWorker-1.10.8+build.35.jar
ReachFix-1.12.2-1.1.3.jar
rebindnarrator-1.0.jar
RedstoneFlux-1.12-2.1.1.1-universal.jar
RoughlyEnoughIDs-2.3.1.jar
Scalar Legacy-1.0.1.jar
serializationisbad-1.5.2.jar
Sledgehammer-1.12.2-2.0.26.jar
SnowVillage-1.12.2-1.0.0.jar
StellarCore-1.5.22.jar
tesla-core-lib-1.12.2-1.0.18.jar
ThermalDynamics-1.12.2-2.5.6.1-universal.jar
ThermalExpansion-1.12.2-5.5.7.1-universal.jar
ThermalFoundation-1.12.2-2.6.7.1-universal.jar
Traverse-1.12.2-1.6.0-69.jar
twilightforest-1.12.2-3.11.1021-universal.jar
TwilightForestFixes-1.0.jar
UniDict-1.12.2-3.0.10.jar
UniversalTweaks-1.12.2-1.20.1.jar
vintagefix-0.7.0.jar
WailaHarvestability-mc1.12-1.1.12.jar
WanionLib-1.12.2-2.91.jar
```
